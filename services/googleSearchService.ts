...
export const searchProjects = async (query: string): Promise<Project[]> => {
  try {
    ...
    const projects: Project[] = data.items
      ...
      .map((item: any) => {
        const langInfo = detectLanguageWithConfidence(item.snippet);
        const tags = extractTagsFromDescription(item.snippet);
        
        // Calculate relevance score based on description quality and matched tags
        const relevanceScore = calculateRelevanceScore(query, item.snippet, tags);
